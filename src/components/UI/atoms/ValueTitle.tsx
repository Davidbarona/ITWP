    ...
    return (
      <div className='value-proposal__group'>
        ...
        <FormattedMessage id={quote} >
          { message => <span className={`value-proposal__title value-proposal__title--color-${styles}`}> {message}</span> }
        </FormattedMessage>
        <FormattedMessage id="value-proposal1" >{message => <h1 className='value-proposal__title'>{message}</h1>}</FormattedMessage>
      </div>
    )
}